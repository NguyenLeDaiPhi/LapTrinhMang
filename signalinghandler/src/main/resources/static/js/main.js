...
async function onConnected() {
    ...
    stompClient.subscribe(`/user/${username}/queue/signals`, onPrivateMessageReceived);

    // Tell your name to the server
    stompClient.send("/app/signal.join",
        {},
...
window.onbeforeunload = () => {
    if (stompClient) {
        // Gracefully leave
        stompClient.send("/app/signal.leave", {}, JSON.stringify({ sender: username, type: 'LEAVE' }));
    }
};
